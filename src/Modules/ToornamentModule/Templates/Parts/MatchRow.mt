...
<component>
  ...
  <property type="MatchInfo" name="match" />
  <property type="GroupInfo" name="group" />
  <property type="RoundInfo" name="round" />
  ...
  <template>
    <Panel x="{{ x }}" y="{{ y }}" width="{{ width }}" height="{{ height }}" bgColor="{{ Theme.UI_BgHighlight }}" overflow="true">
      <label class="text-primary" text="{{ stage.Number }}.{{ group.Number }}.{{ round.Number }}.{{ match.Number }}" valign="center" pos="1 {{ -height/2.0+0.2 }}" />
      <IconButton
              icon="{{ Icons.PlayCircle }}"
              y="{{ -(height-5)/2 }}"
              x="{{ width-6 }}"
              id="btnSelectMatch{{ index }}"
              type="secondary"
              action="MatchManialinkController/SelectMatch/{{ match.TournamentId }}/{{ match.StageId }}/{{ match.Id }}"
