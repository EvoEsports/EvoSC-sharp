<component>
    ...
    <property type="int" name="points"/>
    <property type="bool" name="usePoints"/>

    <template>
        <frame pos="0 {{ y }}">
            <quad
                    bgcolor="{{ Theme.UI_LiveRankingModule_Widget_AccentColor(pos) }}"
                    size="0.7 {{ height }}"
                    pos="0 0"
            />
            <quad
            ...
            >
                ...
                <label
                        text='{{ usePoints ? points : Util.StyledTime(time) }}'
                        class="text-primary"
                        textsize="0.5"
                        pos="{{ width-5.0 }} {{ -height/2.0 + 0.2 }}"
                        valign="center"
                        halign="right"
                        size="16 {{ height }}"
                />
